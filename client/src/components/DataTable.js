...
const DataTable = ({ onRowClick = () => {} }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("https://9bf6-136-233-9-98.ngrok-free.app/users", {
          headers: {
  ...
  const updatePriorityInDB = async (id, newPriority) => {
    try {
      await axios.put("https://9bf6-136-233-9-98.ngrok-free.app/update-priority", {
        _id: id,
  ...
  const updateStatusInDB = async (id, newStatus) => {
    try {
      await axios.put("https://9bf6-136-233-9-98.ngrok-free.app/update-status", {
        _id: id,
        status: newStatus,
      });
    } catch (err) {
